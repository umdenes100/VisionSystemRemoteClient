  ...
  $.get(SERVER_URL, request, data => {
    ...
    canvasses.map(canv => {
      canv.osv = new OSV(
        ...
        data.osv.y,
        data.osv.theta,
        mcanvas.osv.actualWidth / 1000,
        mcanvas.osv.actualHeight / 1000
      );
      canv.obstacles = data.obstacles.map(
        obstacle => new Obstacle(obstacle.x, obstacle.y)
      );
      canv.destination = new Destination(
        data.destination.x,
        data.destination.y
      );
...
function requestSimulation() {
  ...
  let request = {
    ...
    randomization: r,
    distance_sensors: mcanvas.sensors
      .map((sensor, index) => {
        if (sensor.selected) {
          return index;
        }
  ...
  $.get(SERVER_URL, { json: JSON.stringify(request) }, data => {
    ...
    if (data['error'] !== undefined) {
      ...
      if (obstaclesChecked) {
        canvas.obstacles = lastObstacles.map(
          obstacle => new Obstacle(obstacle.x, obstacle.y)
        );
      } else {
        // Obstacles button not checked.
        canvas.obstacles = [].map(
          obstacle => new Obstacle(obstacle.x, obstacle.y)
        );
      }
...
$(document).ready(() => {
  ...
  $('#obstacles').on('click', () => {
    if ($('#obstacles').is(':checked')) {
      ...
      pcanvas.obstacles = lastObstacles.map(
        obstacle => new Obstacle(obstacle.x, obstacle.y)
      );
